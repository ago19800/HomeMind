...
class Notifier:
    ...
    async def send(self, title: str, message: str):
        ...
        # _ha_notify: solo se ha_entity è configurato E non è un notify.telegram_bot_*
        # I servizi telegram_bot di HA non accettano "title" → 400 Bad Request
        entity = self.ha_entity.strip().strip('"').strip("'").strip()
        if entity and not entity.startswith("telegram_bot"):
