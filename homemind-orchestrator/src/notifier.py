...
class Notifier:
    def __init__(self, rest_client, telegram_token: str = "", telegram_chat_id: str = "",
        ...
        self.ha_entity = ha_notify_entity.strip()
    ...
    async def send_html(self, message: str):
        """Notifica con formattazione HTML (per messaggi già formattati con <b>, <i>, ecc.)"""
    ...
    async def _ha_notify(self, title: str, message: str):
        if not self.ha_entity or not self.rest:
            return
        service = self.ha_entity.removeprefix("notify.")
        try:
            await self.rest.call_service("notify", service,
                                         {"title": title, "message": message})
        except Exception as e:
            logger.warning(f"HA notify failed ({service}): {e}")
