...
async def settings_page(request: Request):
    ...
    persons, motions, contacts, powers, energies, distances, temps, humids = [], [], [], [], [], [], [], []
    ...
    for eid, state in state_cache.items():
        ...
        if eid.startswith("person."):
            ...
            if unit in ("W","VA") or "power" in eid.lower() or "watt" in eid.lower():
                ...
                temps.append(item)
            elif dev_class == "humidity" or unit == "%" or "humid" in eid.lower() or "umid" in eid.lower():
                humids.append(item)
...
async function save(){{
  ...
  cfg.temperature_sensors=getSel('tw');
  cfg.humidity_sensors=getSel('hw');
