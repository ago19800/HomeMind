...
async def settings_page(request: Request):
    ...
    if _live_alarm_st in _armed_states:
        _aarm_mode = _live_alarm_st  # usa lo stato live HA
    else:
        _aarm_mode  = cfg.get("alarm_arm_mode", "armed_away")  # fallback a config
  ...
  async function poll(){{
    try{{
      ...
      if(banner){{
        ...
        banner.innerHTML = "<b>Stato attuale allarme HA:</b> " + d.label +
          (d.arm_mode ? " &nbsp;|&nbsp; <i>HomeMind usa modalità: " + d.arm_mode + "</i>" : "");
      }}
      // Aggiorna il select "Modalità di armamento" se l'allarme è armato
      if(d.arm_mode){{
        var sel = document.getElementById("alarm-arm-mode");
        if(sel && sel.value !== d.arm_mode){{
          sel.value = d.arm_mode;
        }}
      }}
    }}catch(e){{}}
    ...
    async def _rebuild():
        while True:
            ...
            try:
                ...
                # ── Aggiorna _was_away per chi è fuori GPS (indipendente dall'allarme)
                for _peid, _pdata in home.persons.items():
                    _pname = _pdata.get("name", "")
                    if _pname and not home._is_home_state(_pdata.get("state", "")):
