...
async def settings_page(request: Request):
    ...
    # ── Variabili Solar Optimizer pre-compilate lato Python ─────────────────
    _so          = cfg.get("solar_optimizer", {})
    _so_en       = "checked" if _so.get("enabled", False) else ""
    _so_min_w    = str(int(_so.get("min_surplus_w", 500)))
    _so_conf     = str(int(_so.get("confirm_minutes", 5)))
    _so_cool     = str(float(_so.get("cooldown_hours", 2)))
    _so_bat_sen  = _so.get("battery_soc_sensor", "")
    _so_bat_thr  = str(int(_so.get("battery_full_threshold", 95)))
    _so_bat_fv   = str(int(_so.get("battery_full_min_fv_w", 300)))
    _so_sun_el   = str(int(_so.get("min_sun_elevation", 10)))
    # Righe appliances solar (dict nome→cfg)
    _so_apps     = _so.get("appliances", {})
    _so_app_rows = ""
    for _san, _sacfg in _so_apps.items():
        if not isinstance(_sacfg, dict): continue
        _sa_sw   = _sacfg.get("switch", "") or ""
        _sa_minw = str(int(_sacfg.get("min_surplus_w", _so.get("min_surplus_w", 500))))
        _sa_auto = "checked" if _sacfg.get("auto_start", False) else ""
        _sa_en   = "checked" if _sacfg.get("enabled", True) else ""
        _so_app_rows += (
            f'<div class="so-row" style="display:flex;gap:6px;align-items:center;'
            f'flex-wrap:wrap;margin-bottom:6px;background:#0f172a;'
            f'border:1px solid #334155;border-radius:8px;padding:8px">'
            f'<input class="so-name" type="text" value="{_san}" placeholder="lavatrice"'
            f' style="width:110px;{INP}">'
            f'<input class="so-sw" type="text" value="{_sa_sw}"'
            f' placeholder="switch.presa (opz.)" style="flex:1;min-width:160px;{INP}">'
            f'<input class="so-minw" type="number" value="{_sa_minw}"'
            f' title="Surplus minimo (W)" style="width:62px;{INP}">'
            f'<label style="display:flex;align-items:center;gap:4px;font-size:.75rem;color:#94a3b8;flex-shrink:0">'
            f'<input type="checkbox" class="so-auto" {_sa_auto}'
            f' style="width:14px;height:14px;accent-color:#22c55e"> Auto</label>'
            f'<label style="display:flex;align-items:center;gap:4px;font-size:.75rem;color:#94a3b8;flex-shrink:0">'
            f'<input type="checkbox" class="so-en" {_sa_en}'
            f' style="width:14px;height:14px;accent-color:#22c55e"> Attivo</label>'
            f'<button type="button" onclick="removeSolarRow(this)"'
            f' style="background:#7f1d1d;border:none;border-radius:5px;'
            f'color:#fca5a5;padding:4px 7px;cursor:pointer;flex-shrink:0">&#x2715;</button>'
            f'</div>'
        )
...
<div class="tabs">
  ...
  <button class="tab" data-tab="o" onclick="showTab('o')">☀️ Solare</button>
</div>
...
<div id="tc-m" class="tc">
  ...
  <div class="sec" style="border-left:4px solid #f87171;margin-top:0">
    ...
    <div class="desc">
      Scegli come HomeMind gestisce l'antifurto quando tutti escono di casa.
    </div>
    <div class="esec">
...
<div id="tc-o" class="tc">
  <div class="sec" style="border-left:4px solid #fbbf24">
    <h2 style="color:#fde68a">☀️ Ottimizzatore Solare</h2>
    <div class="desc">
      Suggerisce (o avvia) elettrodomestici quando c'è surplus fotovoltaico o la batteria è carica.
    </div>
    <div class="tr">
      <div><div class="tl">☀️ Abilita ottimizzatore solare</div><div class="ts">Monitora surplus FV ogni 2 min</div></div>
      <label class="sw"><input type="checkbox" id="so-en" {_so_en}><span class="sl"></span></label>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px">
      <div>
        <span class="el-label">Surplus minimo (W)</span>
        <input type="number" id="so-min-w" value="{_so_min_w}" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      </div>
      <div>
        <span class="el-label">Minuti surplus prima di notificare</span>
        <input type="number" id="so-conf" value="{_so_conf}" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      </div>
      <div>
        <span class="el-label">Cooldown tra notifiche (ore)</span>
        <input type="number" id="so-cool" value="{_so_cool}" step="0.5" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      </div>
      <div>
        <span class="el-label">Elevazione solare minima (°)</span>
        <input type="number" id="so-sun-el" value="{_so_sun_el}" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      </div>
    </div>
  </div>
  <div class="sec" style="border-left:4px solid #f59e0b;margin-top:0">
    <h2 style="color:#fde68a">🔋 Batteria</h2>
    <div class="desc">
      Quando la batteria è carica e il fotovoltaico produce, HomeMind suggerisce di usare il surplus.<br>
      Puoi scrivere qualsiasi <code style="background:#1e293b;padding:1px 4px;border-radius:3px;color:#fde68a">sensor.*</code> — anche se non appare nella lista automatica.
    </div>
    <div style="margin-top:8px">
      <span class="el-label">Sensore SOC batteria (entity_id libero)</span>
      <input type="text" id="so-bat-sen" value="{_so_bat_sen}" placeholder="sensor.inverter_battery_capacity_percent" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      <div style="font-size:.7rem;color:#475569;margin-top:3px">💡 Scrivi direttamente l'entity_id — es. <code>sensor.inverter_battery_voltage</code></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px">
      <div>
        <span class="el-label">Soglia batteria piena (%)</span>
        <input type="number" id="so-bat-thr" value="{_so_bat_thr}" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      </div>
      <div>
        <span class="el-label">FV minimo con batteria piena (W)</span>
        <input type="number" id="so-bat-fv" value="{_so_bat_fv}" style="width:100%;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">
      </div>
    </div>
  </div>
  <div class="sec" style="border-left:4px solid #84cc16;margin-top:0">
    <h2 style="color:#d9f99d">🔌 Elettrodomestici da avviare</h2>
    <div class="desc">
      Colonne: <b>Nome</b> · <b>Switch</b> (opz.) · <b>Surplus min W</b> · <b>Auto</b> (avvio automatico) · <b>Attivo</b>
    </div>
    <div id="so-app-list">{_so_app_rows if _so_app_rows else '<div class="empty">Nessun elettrodomestico configurato</div>'}</div>
    <button type="button" onclick="addSolarRow()"
      style="margin-top:8px;background:#334155;border:none;border-radius:6px;
             color:#94a3b8;padding:6px 14px;font-size:.8rem;cursor:pointer;width:100%">
      + Aggiungi elettrodomestico
    </button>
    <div style="margin-top:10px;padding:10px;background:#1e293b;border-radius:8px;font-size:.73rem;color:#94a3b8;line-height:1.6">
      <b>Auto</b>: HomeMind accende la presa da solo senza chiedere.<br>
      <b>Senza Auto</b>: manda un messaggio Telegram chiedendo conferma.<br>
      <b>Senza Switch</b>: HomeMind notifica ma non agisce sulla presa.
    </div>
  </div>
</div>
...
function removeSolarRow(btn){{var p=btn.parentNode;while(p&&!p.classList.contains("so-row"))p=p.parentNode;if(p)p.remove();}}
function addSolarRow(){{
  var c=document.getElementById("so-app-list");
  var e=c.querySelector(".empty");if(e)e.remove();
  var d=document.createElement("div");
  d.className="so-row";
  d.style.cssText="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:6px;background:#0f172a;border:1px solid #334155;border-radius:8px;padding:8px";
  var s="background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem";
  d.innerHTML='<input class="so-name" type="text" placeholder="lavatrice" style="'+s+';width:110px">'+'<input class="so-sw" type="text" placeholder="switch.presa (opz.)" style="'+s+';flex:1;min-width:160px">'+'<input class="so-minw" type="number" placeholder="W" title="Surplus min W" style="'+s+';width:62px">'+'<label style="display:flex;align-items:center;gap:4px;font-size:.75rem;color:#94a3b8"><input type="checkbox" class="so-auto" style="width:14px;height:14px;accent-color:#22c55e"> Auto</label>'+'<label style="display:flex;align-items:center;gap:4px;font-size:.75rem;color:#94a3b8"><input type="checkbox" class="so-en" checked style="width:14px;height:14px;accent-color:#22c55e"> Attivo</label>'+'<button type="button" onclick="removeSolarRow(this)" style="background:#7f1d1d;border:none;border-radius:5px;color:#fca5a5;padding:4px 7px;cursor:pointer;flex-shrink:0">&#x2715;</button>';
  c.appendChild(d);
}}
function getSolarCfg(){{
  var apps={{}};
  document.querySelectorAll("#so-app-list .so-row").forEach(function(r){{
    var nm=r.querySelector(".so-name").value.trim();
    var sw=r.querySelector(".so-sw").value.trim();
    var mw=parseInt(r.querySelector(".so-minw").value)||500;
    var au=r.querySelector(".so-auto").checked;
    var en=r.querySelector(".so-en").checked;
    if(nm)apps[nm]={{enabled:en,switch:sw||null,min_surplus_w:mw,auto_start:au}};
  }});
  return{{
    enabled:document.getElementById("so-en").checked,
    min_surplus_w:parseInt(document.getElementById("so-min-w").value)||500,
    confirm_minutes:parseInt(document.getElementById("so-conf").value)||5,
    cooldown_hours:parseFloat(document.getElementById("so-cool").value)||2,
    min_sun_elevation:parseInt(document.getElementById("so-sun-el").value)||10,
    battery_soc_sensor:document.getElementById("so-bat-sen").value.trim(),
    battery_full_threshold:parseInt(document.getElementById("so-bat-thr").value)||95,
    battery_full_min_fv_w:parseInt(document.getElementById("so-bat-fv").value)||300,
    appliances:apps
  }};
}}
async function save(){{
  ...
  var ac=getAlarmCfg();if(ac.alarm_panel)cfg.alarm_panel=ac.alarm_panel;cfg.alarm_extra_panels=ac.alarm_extra_panels;cfg.alarm_auto_arm=ac.alarm_auto_arm;cfg.alarm_arm_mode=ac.alarm_arm_mode;
  ...
  cfg.solar_optimizer=getSolarCfg();
  try{{
        ...
        if cfg_path.exists():
            try:
                ...
                UI_FIELDS = {
                    ...
                    "alarm_on_leave", "power_guard", "location_tracker", "solar_optimizer"
                }
