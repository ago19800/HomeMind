...
async def settings_page(request: Request):
    ...
    for _ppers, _pconf in _prox.items():
        ...
        prox_rows += (
            ...
            f'<select class="px-sen" style="flex:1;min-width:180px;{INP}">{_pdist_opts_sel}</select>'
            f'<div style="display:flex;align-items:center;gap:6px;flex-shrink:0">'
            f'<span style="font-size:.75rem;color:#94a3b8">Soglia:</span>'
            f'<input class="px-thr" type="number" value="{_pthr}" style="width:65px;{INP}">'
            f'<span style="font-size:.75rem;color:#94a3b8">m</span>'
            f'</div>'
            f'</div>'
            f'</div>'
        )
...
function addProxRow(){{
  ...
  var d=document.createElement('div');
  d.className='prox-row';d.dataset.person='person.nuovo';
  ...
  d.innerHTML=
    ...
    '<input class="px-pers" type="text" placeholder="person.mario" style="flex:1;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">'+
    ...
    '<input class="px-sen" type="text" placeholder="sensor.casa_mario_distance" style="flex:1;min-width:180px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">'+
    '<div style="display:flex;align-items:center;gap:6px;flex-shrink:0">'+
    '<span style="font-size:.75rem;color:#94a3b8">Soglia:</span>'+
    '<input class="px-thr" type="number" value="100" style="width:65px;background:#0f172a;border:1px solid #334155;border-radius:6px;color:#e2e8f0;padding:6px 8px;font-size:.78rem">'+
    '<span style="font-size:.75rem;color:#94a3b8">m</span>'+
    '</div></div>';
  c.appendChild(d);
}}
function getProxCfg(){{
  ...
  document.querySelectorAll('#prox-list .prox-row').forEach(function(r){{
    ...
    if(pers&&sen)out[pers]={{sensor:sen,threshold_m:thr,stale_check:false}};
  }});
  return out;
}}
...
async def _startup():
    ...
    async def _rebuild():
        while True:
            ...
            home.build_from_cache(state_cache)
            # Controllo anomalia: qualcuno in casa ma allarme armato → disarma
            # Questo gestisce il caso in cui il rientro è avvenuto durante il rebuild
            # e lo startup_check era già passato
            try:
                ...
                if _ast == "armed_away" and not home.everyone_away():
                    ...
                    if _in_casa:
                        ...
                        _log.getLogger("homemind.security").warning(
                            "Rebuild: allarme armed_away ma %s in casa → disarmo emergenza",
                            _in_casa
                        )
