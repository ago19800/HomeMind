...
// Mostra/nascondi toggle entità in base a alarm_auto_arm
(function(){{
  var sel = document.getElementById("alarm-auto-arm");
  var row = document.getElementById("alarm-dis-ent-row");
  if(!sel||!row) return;
  function toggle(){{ row.style.display = sel.value==="disabled" ? "flex" : "none"; }}
  toggle();
...
async function save(){{
  ...
  var ac=getAlarmCfg();if(ac.alarm_panel)cfg.alarm_panel=ac.alarm_panel;cfg.alarm_extra_panels=ac.alarm_extra_panels;cfg.alarm_auto_arm=ac.alarm_auto_arm;cfg.alarm_arm_mode=ac.alarm_arm_mode;cfg.alarm_disable_entities=ac.alarm_disable_entities;
