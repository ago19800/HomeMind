...
async function sendJson(text, msg) {
    try {
        ...
        const r = await fetch('/spazzatura/upload', {
            method: 'POST',
        ...
        if (j.ok) {
            showMsg(msg, '✅ ' + j.message + ' — scrivi /ricarica_spazzatura su Telegram', 'ok');
...
async def spazzatura_upload(request: Request):
    ...
    try:
        ...
        if not isinstance(cal, dict) or not cal:
            return JSONResponse({"ok": False, "error": "JSON vuoto o formato errato"})

        # Salva il file nella directory dell'add-on (dentro il container)
        save_path = Path("/data/spazzatura_calendario.json")
        ...
        if spazzatura:
            ...
            logger.info("Calendario aggiornato via web UI: %d date", len(cal))
...
async def _startup():
    ...
    spazzatura = SpazzaturaManager(ai, notifier, notify_hour=sz_hour)
