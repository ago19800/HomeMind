...
class TelegramBot:
    def __init__(self, token, allowed_chat_ids, ai_callback, action_callback,
                 ...
                 solar_optimizer=None, routine_manager=None, rest_client=None, frigate_client=None):
    ...
    def _quick_command(self, text):
        ...
        if cmd in ("/stato", "stato casa", "stato della casa", "/status", "home status"):
            ...
            # Persone
            persons = {eid: s for eid, s in cache.items() if eid.startswith("person.")}
        ...
        if self.ai_cb and self.context_cb:
            ...
            try:
                ...
                sys_prompt = (
                    ...
                    "  REGOLE CLIMA — segui in base al contesto:\n"
                    "  A) Se vedi uno switch caldaia nel contesto SWITCH/PRESE:\n"
                    "     Accendi: switch.turn_on sullo switch fisico\n"
                    "     Spegni:  switch.turn_off sullo switch fisico\n"
                    "     Temperatura: set_temperature sul termostato (senza hvac_mode prima)\n"
                    "  B) Se hai SOLO un termostato (senza switch separato):\n"
                    "     Accendi: set_hvac_mode heat → poi set_temperature\n"
                    "     Spegni:  set_hvac_mode off\n"
                    "  C) Rispetta SEMPRE i limiti min/max temperature visibili nel contesto CLIMA\n"
                    "     Se il max è 24° non impostare 25° — avvisa l utente del limite\n"
                ...
                if self.ai_action_cb:
                    ...
                    # Se ci sono dati storici, fai un secondo giro AI per analizzarli
                    if history:
                        history_ctx = "\n\n".join(history)
                        followup = (
                            ...
                            f"dell'utente '{user_msg}' in modo chiaro e utile"
                            + (" in italiano." if get_lang() == "it" else " in English.")
                            + " Sii conciso ma informativo. Evidenzia pattern interessanti."
                        )
                        try:
                            analysis = await self.ai_cb(sys_prompt, followup, [])
                            history = ["📊 " + analysis]
                        except Exception as e_h:
                            logger.warning("History analysis error: %s", e_h)
