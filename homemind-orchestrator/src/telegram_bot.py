...
class TelegramBot:
    def __init__(self, token, allowed_chat_ids, ai_callback, action_callback,
                 ...
                 solar_optimizer=None, rest_client=None):
    ...
    async def _handle_update(self, upd):
        ...
        if self.allowed and chat_id not in self.allowed:
            logger.warning("TG: accesso negato chat_id=%s (@%s)", chat_id, username)
            await self._send(chat_id, t("unauthorized"))
            return
        logger.info("TG msg da %s: %s", chat_id, text[:80])
    ...
    def _quick_command(self, text):
        ...
        if cmd in ("/elettrodomestici", "elettrodomestici", "stato elettrodomestici",
                   "/appliances", "appliances", "lavatrice", "lavastoviglie"):
    ...
    async def _ai_reply(self, chat_id, user_msg):
        ...
        if self.ai_cb and self.context_cb:
            ...
            try:
                ...
                sys_prompt = (
                    "Sei HomeMind, assistente AI per la domotica di Agostino.\n"
                    + _lang_instruction + "\n"
                    + _mem_note + "\n"
                    "=== STILE RISPOSTE ===\n"
                    ...
                    "  [GET_HISTORY:person.agostino:12]                             ← presenza oggi\n"
