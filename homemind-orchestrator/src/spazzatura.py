...
class SpazzaturaManager:
    def __init__(self, ai_provider, notifier, notify_hour: int = 20,
                 ...
                 alexa_speakers: list = None,
                 alexa_volume: float = 0.5):
        self.ai           = ai_provider
        self.notifier     = notifier
        self.notify_hour  = max(0, min(23, int(notify_hour)))
        self.notify_enabled = notify_enabled
        self.rest         = rest_client
        self.alexa_enabled  = bool(alexa_enabled)
        self.alexa_speakers = list(alexa_speakers) if alexa_speakers else []
        self.alexa_volume   = max(0.0, min(1.0, float(alexa_volume)))
    ...
    async def _announce_alexa(self, items: list, day_label: str):
        """Annuncia via TTS sugli speaker Alexa configurati (contemporaneamente)."""
        if not self.alexa_enabled or not self.alexa_speakers or not self.rest:
            return
        # Costruisce il messaggio vocale
        if len(items) == 1:
            trash = items[0]
        elif len(items) == 2:
            trash = items[0] + " e " + items[1]
        else:
            trash = ", ".join(items[:-1]) + " e " + items[-1]
        msg = f"Attenzione, {day_label} è il turno di {trash}."

        # Imposta volume (fire-and-forget per ogni speaker)
        async def _speak_on(speaker):
            try:
                # Imposta volume prima dell'annuncio
                await self.rest.call_service(
                    "media_player", "volume_set",
                    {"volume_level": self.alexa_volume},
                    target={"entity_id": speaker}
                )
                # TTS via Alexa Media Player (tts_service dell'integrazione)
                await self.rest.call_service(
                    "notify", "alexa_media",
                    {
                        "target": speaker,
                ...
                logger.info("Alexa annuncio inviato a %s (vol=%.2f)", speaker, self.alexa_volume)
            except Exception as e:
                logger.warning("Alexa annuncio FAIL su %s: %s", speaker, e)

        # Parte in parallelo su tutti gli speaker
        await asyncio.gather(
            *[_speak_on(sp) for sp in self.alexa_speakers],
        ...
        while True:
            try:
                ...
                if items:
                    ...
                    # Annuncio vocale Alexa (opzionale, in parallelo agli speaker)
                    if self.alexa_enabled and self.alexa_speakers:
                        await self._announce_alexa(items, day_label)
                else:
