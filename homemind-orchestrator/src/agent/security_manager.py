...
class SecurityManager:
    def __init__(self, home: HomeModel, rest_client, notifier=None, alarm_code="1234", frigate_client=None, tg_bot=None, cache_cb=None):
        ...
        self._notify_rejected_this_cycle = False  # True dopo NO — reset solo al rientro
        ...
        self._was_away: dict = {}         # person_name → True se proximity ha visto > soglia
        self._pre_disarm_state: dict = {} # person_name → stato allarme PRIMA del disarmo
        ...
        self._last_arm_was_manual: bool = False  # True se l'ultimo armamento è stato manuale
    ...
    async def startup_check(self):
        ...
        # ── Inizializza _was_away al riavvio ──────────────────────────────
        # _was_away si resetta ad ogni riavvio → chi era fuori non riceve benvenuto.
        # Fix: se GPS dice not_home → imposta _was_away=True così al rientro
        # il benvenuto viene inviato correttamente.
        for _peid, _pdata in self.home.persons.items():
            _pname = _pdata.get("name", "")
            if not _pname:
                continue
            _gps_is_home = self.home._is_home_state(_pdata.get("state", ""))
            if not _gps_is_home:
                # GPS dice fuori → stanno per rientrare → prepara benvenuto
                self._was_away[_pname] = True
                logger.info("Startup: %s fuori (GPS) → _was_away=True", _pname)
            else:
                # GPS dice a casa → nessun benvenuto in arrivo
                self._was_away[_pname] = False
    ...
    async def on_state_changed(self, eid: str, old_st: str, new_st: str):
            ...
            if prox["sensor"] == eid:
                try:
                    ...
                    if is_near:
                        ...
                        # Trigger benvenuto SOLO se proximity è PASSATA da far a near
                        # (transizione, non aggiornamento stazionario a 0m)
                        # Usa old_st per capire se era lontano prima
                        try:
                            _old_dist = float(old_st)
                            _was_far = _old_dist > prox["threshold_m"]
                        except (ValueError, TypeError):
                            _was_far = False  # vecchio valore non numerico → non triggerare
                        _gps_home_prox = self.home._is_home_state(
                            self.home.persons.get(person_eid, {}).get("state", "")
                        )
                        if _was_far and _gps_home_prox:
                            # Transizione reale far→near con GPS home → rientro fisico
                            logger.info("📍 %s proximity %.0fm→%.0fm (far→near) + GPS home → benvenuto",
                                        name, _old_dist, dist_m)
                            asyncio.create_task(
        ...
        if domain == "person":
            ...
            if new_st == "pending" and self._pending_welcome:
                # Verifica che chi ha il task pendente sia DAVVERO vicino a casa.
                # Un task residuo da proximity bounce non deve disarmare l'allarme!
                _who_actually_home = [
                    name for name, _t in self._pending_welcome.items()
                    if not _t.done() and any(
                        self.home._person_is_home(peid)
                        for peid, pdata in self.home.persons.items()
                        if pdata.get("name") == name
                    )
                ]
                if _who_actually_home:
                    logger.info(
                        "⚡ Allarme pending + rientro confermato (%s) → disarmo immediato",
                        _who_actually_home
                    )
                    ...
                    asyncio.create_task(
                        self._immediate_disarm(_who_actually_home[0]),
                        name="immediate_disarm_pending"
                    )
                else:
                    _who_pending = list(self._pending_welcome.keys())
                    logger.warning(
                        "⚠️ Allarme pending — task _pending_welcome per %s "
                        "ma nessuno è davvero a casa → NON disarmo (task residuo?)",
                        _who_pending
                    )
                    # Cancella i task residui
                    for _t in list(self._pending_welcome.values()):
                        if not _t.done():
                            _t.cancel()
                    self._pending_welcome.clear()

            # ── Allarme in pending/triggered → foto Frigate + notifica (per tutte le modalità)
            # Per armed_home: è l'UNICO punto in cui HomeMind notifica (i PIR sono ignorati).
            # Per armed_away: rinforzo della notifica già inviata dai PIR.
            if new_st in ("pending", "triggered") and old_st not in ("pending", "triggered"):
                asyncio.create_task(
                    self._notify_alarm_triggered(eid, new_st, old_st),
                    name="alarm_triggered_notify"
                )

            # Reset flag armamento manuale quando viene disarmato
            if new_st == "disarmed" and old_st != "disarmed":
                self._last_arm_was_manual = False
            ...
            # Appena armato → controlla se è stato fatto manualmente o da HomeMind
                ...
                if _from_triggered:
                    ...
                    self._last_arm_was_manual = True  # blocca disarmo emergenza del rebuild
                    asyncio.create_task(
                    ...
                    self._armed_by_homemind = False
                    self._last_arm_was_manual = False  # armamento HomeMind, non manuale
    ...
    async def _handle_presence(self, eid: str, old_st: str, new_st: str):
        async with self._lock:
            ...
            # STESSA protezione bounce del blocco is_leaving: se GPS era home < 5 min fa
            # → NON aggiornare left_ts (bounce al rientro)
            if is_home_old and not gps_is_home_new:
                _last_home_ts_early = self._last_gps_home_ts.get(name, 0)
                _since_early = time.time() - _last_home_ts_early
                if _since_early >= 300:  # non è un bounce → aggiorna left_ts
                    self._left_ts[name] = time.time()
                    logger.info("👤 %s: partenza GPS registrata (proximity potrebbe essere stale)", name)
                else:
                    logger.info("👤 %s: partenza GPS registrata ma left_ts NON aggiornato "
                                "(GPS era home %.0fs fa, bounce)", name, _since_early)
    ...
    async def _all_left(self, trigger: str):
        ...
        # ── Modalità NOTIFY: chiede conferma PRIMA di fare qualsiasi cosa ───
        # ORDINE CORRETTO: notifica → attendi risposta → poi (se sì) spegni entità + arma
        if _arm_mode_cfg == "notify":
            # Se l'utente ha già rifiutato in questa uscita → non riproporre mai
            # Viene resettato solo quando qualcuno rientra a casa
            if self._notify_rejected_this_cycle:
                logger.info("ARM NOTIFY SKIP: utente ha già rifiutato in questa uscita")
                return
            # Cooldown anti-bounce GPS: evita doppia notifica in pochi secondi
            NOTIFY_CONFIRM_COOLDOWN = 30  # 30s — solo anti-bounce, non anti-spam
            if time.time() - self._last_arm_notify_ts < NOTIFY_CONFIRM_COOLDOWN:
                logger.info("ARM NOTIFY SKIP: anti-bounce attivo")
                return
            self._last_arm_notify_ts = time.time()
            self._arm_notify_in_progress = True
            confirmed = False
            logger.info("ARM NOTIFY: chiedo conferma (timeout 10 min)...")
            if self._tg_bot:
                try:
                    who_away = self.home.who_is_away()
                    ...
                    question = (
                        ...
                        f"<i>Hai 10 minuti per rispondere.</i>"
                    )
                    confirmed = await self._tg_bot.ask_alarm_confirm(question, timeout=600)
                except Exception as e_ask:
                    logger.warning("ARM NOTIFY: errore: %s — armo automaticamente", e_ask)
                    confirmed = True
            else:
                confirmed = True

            self._arm_notify_in_progress = False
            self._last_arm_ts = time.time()

            if not confirmed:
                logger.info("ARM NOTIFY: rifiutato — non riproporrò più in questa uscita")
                self._notify_rejected_this_cycle = True  # blocca fino al prossimo rientro
                if self.notifier:
                    try:
                        await self.notifier.send_html(
                            f"🔕 <b>Antifurto NON attivato</b> — {now_local().strftime('%H:%M')}\n"
                            f"<i>Non verrà più riproposto per questa uscita.</i>"
                        )
                    except Exception as e:
                        logger.warning("Notifica rifiuto FAIL: %s", e)
                return
            logger.info("ARM NOTIFY: confermato — procedo con spegnimento entità e armamento")
    ...
    async def _someone_arrived(self, name: str):
        ...
        # Reset flag rifiuto notify — al prossimo ciclo di uscita ripropone
        self._notify_rejected_this_cycle = False
        ...
        # ── Cattura stato allarme PRIMA del disarmo ──────────────────────────
        # Salvato qui perché dopo il disarmo lo stato è già "disarmed"
        # e la notifica benvenuto non saprebbe più se era armato.
        _, _alarm_now = self.home.primary_alarm()
        if _alarm_now in ("armed_away","armed_home","armed_night","armed_vacation"):
            self._pre_disarm_state[name] = _alarm_now
            logger.debug("👤 %s: stato allarme pre-disarmo salvato: %s", name, _alarm_now)
    ...
    async def _delayed_welcome_notify(self, name: str, delay: int):
        ...
        # 3. Proximity controls:
        #    a) non deve dire lontano (evita benvenuto a 3km con GPS bounce)
        #    b) dato non deve essere troppo vecchio (sensore morto → non fidarsi)
        for _peid, _prox in self.home.proximity_sensors.items():
            ...
            _ls  = _prox.get("last_seen")
            # Se proximity dice "vicino" ma il dato è vecchio > 30 min,
            # non è una conferma valida — il sensore potrebbe essere offline
            if _ls is not None and _cm is not None and float(_cm) <= _thr:
                _age_min = (time.time() - float(_ls)) / 60
                if _age_min > 30:
                    logger.info(
                        "Benvenuto %s ANNULLATO: proximity stale (%.0f min fa, "
                        "sensore probabilmente offline)", name, _age_min)
                    return
        ...
        if self.notifier:
            try:
                ...
                # Usa lo stato allarme catturato PRIMA del disarmo
                # (dopo il disarmo lo stato è già "disarmed" → non sapremmo più)
                _alarm_before = self._pre_disarm_state.pop(name, None)
                _was_armed = _alarm_before in (
                    "armed_away", "armed_home", "armed_night", "armed_vacation"
                )
                # Messaggio base benvenuto
                _msg = (
                    name + " rientrato/a\n"
                    "In casa: " + (", ".join(who) or name) + "\n"
                    "Ora: " + now_local().strftime("%H:%M")
                )
                # Aggiungi info allarme se era armato
                if _was_armed:
                    _mode_labels = {
                        "armed_away":    "🏃 via da casa",
                        "armed_home":    "🏠 perimetrale",
                        "armed_night":   "🌙 notturno",
                        "armed_vacation":"✈️ vacanza",
                    }
                    _mode_str = _mode_labels.get(_alarm_before, _alarm_before)
                    _msg += "\n🔓 Antifurto disarmato (era: " + _mode_str + ")"
    ...
    async def _notify_alarm_triggered(self, alarm_eid: str, new_st: str, old_st: str):
        """Invia foto Frigate e notifica Telegram quando l'hardware va in pending/triggered.
        Attivo per armed_home/night (unico punto di notifica in modalità perimetrale).
        Rispetta alarm_auto_arm=disabled + alarm_disable_notify=false (silenzio totale).
        """
        await asyncio.sleep(1)  # piccolo delay per stabilizzare lo stato

        # Rispetta le preferenze utente:
        _auto_arm = getattr(self.home, "_alarm_auto_arm", "auto")
        _dis_notify = getattr(self.home, "_alarm_disable_notify", True)
        _dis_entities = getattr(self.home, "_alarm_disable_entities", True)

        # disabled + alarm_disable_notify=False → silenzio totale
        if _auto_arm == "disabled" and not _dis_notify and not _dis_entities:
            logger.debug("_notify_alarm_triggered: silenzio totale (disabled+no_notify)")
            return
        # disabled + alarm_disable_entities=False → nessuna notifica allarme hardware
        # (l'utente gestisce l'allarme da solo, HomeMind non deve interferire)
        if _auto_arm == "disabled" and not _dis_entities:
            logger.debug("_notify_alarm_triggered: disabled+no_entities → skip notifica hardware")
            return

        _, alarm_st = self.home.primary_alarm()
        arm_mode = getattr(self.home, "_alarm_arm_mode", "armed_away")
        is_night = now_local().hour >= 22 or now_local().hour < 7
        icon = "🌙" if is_night else "🚨"
        away = self.home.who_is_away()

        logger.warning("🚨 Allarme hardware %s → %s — invio notifica e foto", old_st, new_st)

        if self.notifier:
            try:
                label = "ALLARME IN ENTRATA" if new_st == "pending" else "ALLARME ATTIVO"
                await self.notifier.send(
                    f"{icon} {label}",
                    f"Modalità: {arm_mode}\n"
                    f"Ora: {now_local().strftime('%H:%M:%S')}\n"
                    + (f"Assenti: {', '.join(away)}" if away else "Tutti fuori")
                )
            except Exception as e:
                logger.warning("Notifica alarm triggered FAIL: %s", e)

        # Foto Frigate — tutte le camere disponibili
        if self._frigate and self._frigate.is_ready() and self._tg_bot:
            try:
                FRIGATE_COOLDOWN = 60
                now_ts = asyncio.get_event_loop().time()
                for cam_name in self._frigate.all_cameras():
                    last = self._frigate_sent.get(cam_name, 0)
                    if now_ts - last >= FRIGATE_COOLDOWN:
                        ...
                        logger.info("Frigate snapshot alarm: %s", cam_name)
            except Exception as e:
                logger.warning("Frigate alarm snapshot FAIL: %s", e)
    ...
    async def _schedule_motion(self, eid: str):
        ...
        # LOGICA SKIP DEFINITIVA:
        # armed_home/night → HomeMind ignora sempre i propri PIR Zigbee.
        #   In modalità perimetrale l'hardware gestisce le sue zone.
        #   HomeMind invierà foto/notifica solo quando l'hardware va in pending/triggered.
        # armed_away/vacation → triggera se casa vuota
        if alarm_st in ("armed_home", "armed_night"):
            logger.debug("Motion skip %s: %s — PIR ignorati, solo perimetrali hardware", eid, alarm_st)
            return
        if alarm_st in ("armed_away", "armed_vacation"):
            if not self.home.everyone_away() and alarm_st not in ("triggered", "pending"):
                logger.debug("Motion skip %s: %s + persone in casa", eid, alarm_st)
                return
        ...
        # Notifica e Frigate snapshot inviati solo in _fire_motion (dopo conferma cross-ref)
    ...
    async def _fire_motion(self, eid: str):
        ...
        # Stessa logica di _schedule_motion:
        # armed_home/night → annulla sempre (PIR ignorati)
        # armed_away/vacation + persone in casa → annulla
        if alarm_st in ("armed_home", "armed_night"):
            logger.info("Motion %s annullato: %s — PIR ignorati", eid, alarm_st)
            self._recent_motion.pop(eid, None)
            return
        if alarm_st in ("armed_away", "armed_vacation") \
                and not self.home.everyone_away() \
                and alarm_st not in active_states:
            logger.info("Motion %s annullato: %s + persone in casa", eid, alarm_st)
            self._recent_motion.pop(eid, None)
            return
    ...
    async def _sync_alarm_mode_to_config(self, new_mode: str):
        ...
        # Notifica discreta "armato manualmente" — rispetta alarm_disable_notify
        _auto_arm = getattr(self.home, "_alarm_auto_arm", "auto")
        _dis_notify = getattr(self.home, "_alarm_disable_notify", True)
        _dis_entities = getattr(self.home, "_alarm_disable_entities", True)
        # Con disabled + notify=false → silenzio totale, non notificare
        if _auto_arm == "disabled" and not _dis_notify and not _dis_entities:
            logger.debug("_sync_alarm_mode: silenzio totale — skip notifica")
            return
        if self.notifier:
            mode_labels = {
