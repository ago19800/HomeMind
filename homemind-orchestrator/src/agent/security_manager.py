...
class SecurityManager:
    ...
    async def _handle_presence(self, eid: str, old_st: str, new_st: str):
        async with self._lock:
            ...
            is_arriving = is_home_new and not is_home_old        # proximity/GPS dice in casa
            ...
            if is_leaving:
                ...
                _sleep_sec = 30 if _stale_d else 8
                logger.info("👤 %s uscito — attendo %ds prima di verificare casa vuota%s",
                            name, _sleep_sec, " (proximity stale)" if _stale_d else "")
    ...
    def _check_multi_source(self, person_eid: str) -> tuple[bool, str]:
        ...
        # Fonte 3: Proximity distanza (opzionale)
        # Logica "near" vs "far":
        #   - FAR (dist > threshold): conta sempre come AWAY — sensor si è aggiornato
        #   - NEAR + freschi < 3min: conta come HOME — persona davvero vicina di recente
        #   - NEAR + 3-max_age*2 min: NEUTRALE — sensor non aggiornato da troppo,
        #     non possiamo fidarci che la persona sia ancora lì
        #   - NEAR + > max_age*2: esclusa (stale assoluto)
        # Questo evita che proximity ferma a 0m blocchi l'armamento dopo 5+ minuti
        PROX_NEAR_FRESH_SEC = 180  # 3 minuti: finestra in cui proximity "near" è affidabile
        prox_away = None
        current_m = prox.get("current_m")
        last_seen = prox.get("last_seen")
        max_age   = int(prox.get("max_age_min", 30)) * 60
        if current_m is not None and last_seen is not None:
            data_age  = time.time() - last_seen
            threshold = float(prox.get("threshold_m", 100))
            is_far    = float(current_m) > threshold
            if data_age > max_age * 2:
                # Stale assoluto — esclusa
                logger.info("MULTI-SOURCE: proximity stale assoluto (%.0fmin) — esclusa", data_age/60)
            elif is_far:
                # Sensore aggiornato e dice lontano → affidabile come AWAY
                prox_away = True
                logger.debug("MULTI-SOURCE: proximity FAR (%.0fm > %.0fm) → away", current_m, threshold)
            elif data_age <= PROX_NEAR_FRESH_SEC:
                # Sensore recente (< 3min) e dice vicino → affidabile come HOME
                prox_away = False
                logger.debug("MULTI-SOURCE: proximity NEAR fresh (%.0fm, %.0fs) → home", current_m, data_age)
            else:
                # Sensore dice vicino MA dati 3+ min → NEUTRALE
                # Non aggiunto alle sources → non blocca, non aiuta
                logger.info(
                    "MULTI-SOURCE: proximity NEAR stale (%.0fm, %.0fmin) → neutrale (no conteggio)",
                    current_m, data_age / 60
                )
