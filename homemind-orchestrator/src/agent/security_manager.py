...
class SecurityManager:
    def __init__(self, home: HomeModel, rest_client, notifier=None, alarm_code="1234", frigate_client=None, tg_bot=None, cache_cb=None):
        ...
        self._left_ts: dict = {}             # person_name → timestamp ultima uscita (aggiornato anche da bounce)
        self._real_departure_ts: dict = {}   # person_name → timestamp partenza CONFERMATA (non azzerato da bounce)
        ...
        async with self._lock:
            ...
            if is_leaving:
                ...
                if _since_gps_home < 300:  # GPS è andato home negli ultimi 5 min → bounce
                    ...
                    self._left_ts[name] = time.time()
                    # _real_departure_ts: aggiornato solo se proximity > soglia
                    # oppure prima partenza — mai azzerato da bounce GPS durante rientro
                    _prox_dep = self.home.proximity_sensors.get(eid, {})
                    _cm_dep   = _prox_dep.get("current_m")
                    _thr_dep  = float(_prox_dep.get("threshold_m", 100))
                    _prox_far = _cm_dep is not None and float(_cm_dep) > _thr_dep
                    if _prox_far or not self._real_departure_ts.get(name, 0):
                        self._real_departure_ts[name] = time.time()
    ...
    async def _someone_arrived(self, name: str):
        ...
        # Usa _real_departure_ts (partenza confermata da proximity) se disponibile,
        # altrimenti left_ts. Questo evita che i bounce GPS durante il rientro
        # rendano away_minutes < 5 min → benvenuto SKIP.
        left_ts      = self._left_ts.get(name, 0.0)
        real_dep_ts  = self._real_departure_ts.get(name, 0.0)
        _dep_ts      = real_dep_ts if real_dep_ts > 0 else left_ts
        away_minutes = (time.time() - _dep_ts) / 60 if _dep_ts else 999
    ...
    async def _immediate_disarm(self, name: str):
        ...
        # Disarma anche partizioni extra
    ...
    async def _delayed_welcome_notify(self, name: str, delay: int):
        ...
        # Reset real_departure_ts: benvenuto inviato = "tornato a casa"
        # Prossimi bounce GPS avranno away_minutes bassi → no spam
        self._real_departure_ts[name] = 0.0
