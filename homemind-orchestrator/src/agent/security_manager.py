...
class SecurityManager:
    def __init__(self, home: HomeModel, rest_client, notifier=None, alarm_code="1234"):
        self.home       = home
    ...
    async def _handle_presence(self, eid: str, old_st: str, new_st: str):
        async with self._lock:
            ...
            logger.info("👤 %s: %s→%s | home_new=%s home_old=%s",
                        name, old_st, new_st, is_home_new, is_home_old)
    ...
    async def _all_left(self, trigger: str):
        ...
        if not alarm_eid:
            ...
            try:
                logger.info("Armamento allarme %s (stato attuale: %s)...", alarm_eid, alarm_st)
                await self.rest.call_service(
                    "alarm_control_panel", "alarm_arm_away",
                    {"code": self.alarm_code},
                    target={"entity_id": alarm_eid}
                )
                done.append("Allarme ARMATO")
                logger.info("✅ Allarme armato: %s", alarm_eid)
            except Exception as e:
    ...
    async def _someone_arrived(self, name: str):
        ...
        if elapsed_since_last >= WELCOME_COOLDOWN and away_minutes >= 5:
            notify_task = asyncio.create_task(
                self._delayed_welcome_notify(name, delay=60),
                name=f"welcome_notify_{name}"
            )
            self._pending_welcome_notify[name] = notify_task
            logger.info("👤 %s: benvenuto schedulato (via %.0f min, ultimo benvenuto %.0f min fa)",
