...
class ConfigEditor:
    ...
    def detect_wizard(self, text: str) -> Optional[dict]:
        ...
        if any(p in t for p in ('aggiungi.*power.guard', 'aggiungi.*powerguard',
                                  'add.*power.guard', 'aggiungi elettrodomestico.*guard')):
            m = re.search(r'aggiungi.*power.guard', t) or re.search(r'add.*power.guard', t)
            # Estrai nome se già nel testo
    ...
    def _apply_wizard_result(self, cfg: dict, intent: dict) -> tuple[bool, str]:
        ...
        if wtype == "proximity_add":
            ...
            apps.append({"name": name, "switch": switch})
            pg["appliances_priority"] = apps
            if "appliances" in pg:
                del pg["appliances"]
            cfg["power_guard"] = pg
    ...
    async def handle_wizard_step(self, text: str) -> bool:
        ...
        # ── Annulla wizard
        if t.lower() in ("annulla", "cancel", "stop", "esci", "no"):
            self._wizard = None
        ...
        # ── WIZARD: Power Guard Appliance Add ─────────────────────────────
            ...
            if step == "name":
                ...
                await self.notifier.send_html(
                    ...
                    f"⚙️ Entity_id dello switch da controllare:\n"
                    f"Esempio: <code>switch.presa_forno</code>"
                )
                return True

            elif step == "switch":
                m = re.search(r'switch\.\w+', t.lower())
                if not m:
                    await self.notifier.send_html(
                        "⚙️ Scrivi l'entity_id dello switch\n"
                        "Esempio: <code>switch.presa_forno</code>"
                    )
                    return True
                data["switch"] = m.group(0)
                # Applica
                self._wizard = None
    ...
    def get_config_display(self) -> str:
        ...
        if greeting:
            # Normalizza greeting per display
            _gw = greeting.split()
            _gkw = {"buongiorno","ciao","hello","good","salve","hey","benvenuto","morning"}
        ...
        sections_std = ["meteo", "energia", "stato casa", "riparazioni", "spazzatura", "consiglio", "routine"]
