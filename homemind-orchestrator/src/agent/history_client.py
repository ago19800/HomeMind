    ...
    async def energy_summary(self, period: str = "oggi") -> str:
        ...
        if period.lower() == "ieri":
            ...
            if "produzione_fv" in available:
                ...
                try:
                    fv_now = float(fv_raw)
                    if fv_now > 1000:  # sensore cumulativo
                        # Cerca il valore di stamattina (prime letture oggi)
                        try:
                            hist_fv = await self.rest.get_history([fv_eid], hours=20)
                            entries_fv = hist_fv.get(fv_eid, [])
                            from utils.timezone_helper import local_tz as _ltz
                            today_local = datetime.now(_ltz()).date()
                            today_vals = []
                            for e in entries_fv:
                                ts_r = e.get("last_changed", "")
                                try:
                                    ts_dt = datetime.fromisoformat(ts_r.replace("Z","+00:00"))
                                    if ts_dt.astimezone(_ltz()).date() == today_local:
                                        today_vals.append(float(e.get("state",0)))
                                except Exception:
                                    pass
                            if today_vals:
                                fv_oggi_delta = round(fv_now - min(today_vals), 2)
                        except Exception:
                            pass
                except (ValueError, TypeError):
                    pass
            ...
            for cat, eid in available.items():
                ...
                try:
                    ...
                    if cat == "produzione_fv" and fv_oggi_delta is not None:
                        ...
                        lines.append(f"{icon} <b>{lbl}:</b> {fv_oggi_delta:.2f} {u} <i>(oggi)</i>".strip())
                    else:
                        values[cat] = fv
