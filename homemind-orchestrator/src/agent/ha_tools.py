...
class HATools:
    ...
    def full_context(self) -> str:
        ...
        if temps:
            ...
            try:
                ...
                import json as _j2
                _tp2 = _P2("/config/homemind_patches/person_config.json")
                if _tp2.exists():
                    _wp = _j2.loads(_tp2.read_text(encoding="utf-8-sig")).get("temperature_sensors", [])
            except Exception:
                pass
            _temp_header = "--- TEMPERATURE (mostra SOLO questi, ignora altre temperature nel contesto) ---" if _wp else "--- TEMPERATURE ---"
            parts += [_temp_header, temps, ""]

        # Umidità
        humids = self._humidities()
        if humids:
            import json as _jh, pathlib as _ph
            _hwl = []
            try:
                _htp = _ph.Path("/config/homemind_patches/person_config.json")
                if _htp.exists():
                    _hwl = _jh.loads(_htp.read_text(encoding="utf-8-sig")).get("humidity_sensors", [])
            except Exception:
                pass
            _hum_header = "--- UMIDITÀ (mostra SOLO questi) ---" if _hwl else "--- UMIDITÀ ---"
            parts += [_hum_header, humids, ""]
        ...
        # Switch (prese smart, relè, caldaie, ecc.)
        sw_block = self._switches_block()
        if sw_block:
            parts += ["--- SWITCH / PRESE ---", sw_block, ""]

        # Tapparelle / Tende / Cover
        cover_block = self._covers_block()
        if cover_block:
            parts += ["--- TAPPARELLE / TENDE ---", cover_block, ""]
    ...
    def _temperatures(self) -> str:
        ...
        _whitelist = []
        try:
            _tp = _P("/config/homemind_patches/person_config.json")
            if _tp.exists():
                _tc = _j.loads(_tp.read_text(encoding="utf-8-sig"))
                _whitelist = _tc.get("temperature_sensors", [])
        except Exception:
            pass

        TEMP_EXCLUDE = ("cpu", "processor", "system", "proxmox", "raspberry",
                        "board", "soc", "thermal", "battery", "batteria",
                        "apparent", "outside_temperature", "inside_temperature",
                        "weather", "meteo", "sm-", "samsung", "tablet", "phone")
        lines = []
        for eid, s in self.cache.items():
            ...
            if v in ("unavailable","unknown",""):
                continue
            # Se whitelist configurata → mostra solo quelli
            if _whitelist and eid not in _whitelist:
                continue
            # Se no whitelist → applica filtro esclusione automatico
            if not _whitelist:
                name_low = s.get("attributes",{}).get("friendly_name", eid).lower()
                if any(k in eid.lower() or k in name_low for k in TEMP_EXCLUDE):
                    continue
    ...
    def _humidities(self) -> str:
        """Restituisce i sensori umidità per il contesto AI, rispettando la whitelist."""
        import json as _j
        from pathlib import Path as _P
        _whitelist = []
        try:
            _tp = _P("/config/homemind_patches/person_config.json")
            if _tp.exists():
                _whitelist = _j.loads(_tp.read_text(encoding="utf-8-sig")).get("humidity_sensors", [])
        except Exception:
            pass

        HUMI_EXCLUDE = ("battery", "batteria", "cpu", "processor", "system",
                        "proxmox", "raspberry", "board", "soc", "pm25", "pm2",
                        "co2", "voc", "lux", "illuminance", "signal", "rssi")
        lines = []
        for eid, s in self.cache.items():
            dc = s.get("attributes", {}).get("device_class", "")
            unit = s.get("attributes", {}).get("unit_of_measurement", "")
            # Deve essere un sensore umidità: device_class=humidity OPPURE
            # unità % e "humid"/"umid" nell'eid o nel nome
            name_low = s.get("attributes", {}).get("friendly_name", eid).lower()
            is_humidity = (dc == "humidity") or (
                unit in ("%", "% RH", "%RH") and
                ("humid" in eid.lower() or "umid" in eid.lower() or
                 "humid" in name_low or "umid" in name_low)
            )
            if not is_humidity:
                continue
            v = s.get("state", "")
            if v in ("unavailable", "unknown", ""):
                continue
            # Whitelist: mostra solo quelli configurati
            if _whitelist and eid not in _whitelist:
                continue
            # Senza whitelist: escludi sensori non fisici
            if not _whitelist:
                if any(k in eid.lower() or k in name_low for k in HUMI_EXCLUDE):
                    continue
            name = s.get("attributes", {}).get("friendly_name", eid)
            u    = unit or "%"
            lines.append(name + ": " + v + " " + u)
    ...
    def _climate_block(self) -> str:
        lines = []
        climate_cfg = getattr(self.home, "_climate_config", {})
        for eid, v in self.home.climate.items():
            ...
            hvac  = v["state"]
            # Priorità: person_config → attributi HA
            cfg_this = climate_cfg.get(eid, {})
            min_t = cfg_this.get("min_temp") or attrs.get("min_temp", "")
            max_t = cfg_this.get("max_temp") or attrs.get("max_temp", "")
            range_str = f" | range={min_t}°-{max_t}° (max configurato)" if min_t and max_t else ""
            # Modalità HVAC supportate — fondamentale per SmartIR
            hvac_modes = attrs.get("hvac_modes", [])
            # Determina tipo: SmartIR rilevato automaticamente o da config
            # SmartIR usa climate.turn_on/turn_off invece di set_hvac_mode
            cfg_type = cfg_this.get("type", "").lower()  # override manuale: "smartir" o "standard"
            is_smartir = (
                cfg_type == "smartir" or
                bool(attrs.get("controller_data") or attrs.get("controller_model")) or
                (not hvac_modes and cfg_type != "standard")  # nessuna hvac_mode → probabilmente SmartIR
            )
            # Costruisci stringa modalità
            if hvac_modes:
                modes_str = ",".join(hvac_modes)
            else:
                modes_str = "sconosciute"
            line = (f"{v['name']} ({eid}): {hvac} | corrente={cur}° target={tgt}°{range_str}"
                    f" | hvac_modes=[{modes_str}]")
            if is_smartir:
                line += " | tipo=SmartIR (usa climate.turn_on/turn_off)"
            elif not hvac_modes:
                line += " | tipo=standard"
            # Aggiungi info switch caldaia se configurato
            cfg = climate_cfg.get(eid, {})
            if cfg.get("switch"):
                sw = cfg["switch"]
                sw_st = self.cache.get(sw, {}).get("state", "?")
                sw_name = self.cache.get(sw, {}).get("attributes", {}).get("friendly_name", sw)
                line += f" | interruttore={sw_name}({sw}) stato={sw_st}"
            lines.append(line)
        # Aggiungi entità clima standalone non in self.home.climate
        for eid, cfg in climate_cfg.items():
            if eid not in self.home.climate and "switch" in cfg:
                sw = cfg["switch"]
                sw_st = self.cache.get(sw, {}).get("state", "?")
                sw_name = self.cache.get(sw, {}).get("attributes", {}).get("friendly_name", sw)
                label = cfg.get("name", eid)
                lines.append(f"{label}: interruttore={sw_name}({sw}) stato={sw_st}")
        return "\n".join(lines)

    def _switches_block(self) -> str:
        """Elenca switch utili (esclude quelli di servizio/sistema)."""
        EXCLUDE_KW = (
            "updater", "recorder", "logger", "automation", "script",
            "input_", "timer", "counter", "zone", "group",
            "cast", "media_", "hassio", "supervisor", "homeassistant"
        )
        lines = []
        for eid, v in self.home.switches.items():
            eid_low = eid.lower()
            if any(k in eid_low for k in EXCLUDE_KW):
                continue
            state_str = "ON" if v["state"] == "on" else "off"
            lines.append(f"{v['name']} ({eid}): {state_str}")
        return "\n".join(lines)

    def _covers_block(self) -> str:
        """Elenca tapparelle, tende e coperture con stato e posizione reale.
        
        IMPORTANTE: In HA il campo state delle cover può essere inaffidabile.
        Usa sempre current_position (0=completamente chiuso, 100=completamente aperto).
        """
        if not self.home.covers:
            return ""
        lines = []
        for eid, v in self.home.covers.items():
            # Leggi attributi aggiornati dalla cache (fonte di verità)
            s     = self.cache.get(eid, {})
            attrs = s.get("attributes", {})
            name  = attrs.get("friendly_name", v.get("name", eid))
            state = s.get("state", v.get("state", "unknown"))
            # current_position è il valore REALE: 0=chiuso, 100=aperto
            pos   = attrs.get("current_position", v.get("position"))
            tilt  = attrs.get("current_tilt_position", v.get("tilt"))

            # Determina lo stato leggibile dall'AI
            if pos is not None:
                if pos == 0:
                    status = f"CHIUSA (position=0%)"
                elif pos == 100:
                    status = f"APERTA (position=100%)"
                elif pos < 30:
                    status = f"quasi chiusa (position={pos}%)"
                elif pos > 70:
                    status = f"quasi aperta (position={pos}%)"
                else:
                    status = f"parzialmente aperta (position={pos}%)"
            else:
                # Fallback sullo state HA
                state_map = {
                    "open":    "APERTA",
                    "closed":  "CHIUSA",
                ...
                status = state_map.get(state.lower(), state)

            line = f"{name} [{eid}]: {status}"
            if tilt is not None:
                line += f" | inclinazione={tilt}%"
            lines.append(line)
        return "\n".join(lines)
