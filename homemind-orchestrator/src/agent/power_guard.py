...
class PowerGuard:
    ...
    def _find_target(self) -> Optional[dict]:
        ...
        for app in self._appliances:
            # Supporta sia "switch" che "climate" come entità controllabile
            entity = app.get("switch", "") or app.get("climate", "")
            if not entity:
                continue
            st = cache.get(entity, {}).get("state", "off")
            # Switch: deve essere "on" — Climate: deve essere acceso (non "off")
            is_on = (st == "on") if entity.startswith("switch.") else (st not in ("off", "unavailable", "unknown", ""))
            if not is_on:
                continue
            if now_ts - self._last_action.get(entity, 0) < COOLDOWN_SEC:
                continue
            return app
        return None
    ...
    async def _turn_off(self, app: dict):
        sw      = app.get("switch", "")
        climate = app.get("climate", "")
        entity  = sw or climate
        if not entity or not self.rest:
            return
        try:
            if climate:
                # Spegni climatizzatore via hvac_mode=off
                await self.rest.call_service(
                    "climate", "set_hvac_mode", {"hvac_mode": "off"},
                    target={"entity_id": climate}
                )
                logger.info("PowerGuard: climatizzatore spento %s (%s)", app.get("name"), climate)
            else:
                await self.rest.call_service(
                    "switch", "turn_off", {},
                    target={"entity_id": sw}
                )
                logger.info("PowerGuard: switch spento %s (%s)", app.get("name"), sw)
            self._last_action[entity] = time.time()
        except Exception as e:
            logger.error("PowerGuard: errore spegnendo %s: %s", entity, e)
    ...
    def get_status(self) -> str:
        ...
        for i, app in enumerate(self._appliances, 1):
            sw   = app.get("switch", "") or app.get("climate", "")
