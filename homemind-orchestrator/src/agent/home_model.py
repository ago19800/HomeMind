...
class HomeModel:
    ...
    def _load_config(self):
        ...
        try:
            ...
            alarm_panel = cfg.get("alarm_panel", "").strip()
            if alarm_panel and "." in alarm_panel:
                self._alarm_panel_override = alarm_panel
                logger.info("  alarm_panel: %s (personalizzato)", alarm_panel)
            else:
    ...
    def build_from_cache(self, cache: dict):
        ...
        for eid, state in cache.items():
            ...
            if domain == "person":
                ...
                self.covers[eid] = {"name": name, "state": st}
            elif domain == "switch":
            ...
            if sensor_eid in cache:
                ...
                try:
                    ...
                    if stale:
                        ...
                        # stale_check=false: dati vecchi ma override SEMPRE attivo
                        import time as _t
                        prox["current_m"] = dist_m          # ← mantieni valore!
                        # Imposta last_seen basandosi su last_updated della cache
                        # Così _person_is_home sa l'età reale dei dati
                        prox["last_seen"] = _t.time() - (age_min * 60) if age_min else None
                        is_near = dist_m <= prox["threshold_m"]
                        if is_near:
                            prox["last_near_ts"] = _t.time()  # ← sticky attivo!
                        logger.info("PROXIMITY init: %s = %.0fm (dati %.0f min fa) — stale_check=false, override SEMPRE ATTIVO → near=%s",
                                    person_name, dist_m, age_min, is_near)
                    else:
    ...
    def update_state(self, eid: str, new_st: str):
        for d in [self.persons, self.motion_sensors, self.contact_sensors, self.lights,
                  self.climate, self.alarm_panels, self.covers, self.switches]:
            if eid in d:
                d[eid]["state"] = new_st
                return
