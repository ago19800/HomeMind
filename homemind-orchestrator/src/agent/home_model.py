...
class HomeModel:
    ...
    def _person_is_home(self, person_eid: str) -> bool:
        ...
        if prox:
            ...
            # Limite stale: se GPS dice fuori → max 90min (evita override stale su uscita)
            # Se GPS dice in casa → max 4h (protegge da GPS bounce al rientro)
            ...
            if current_m is not None and data_age < MAX_STALE_SEC:
                ...
                logger.info("Proximity %s: dati troppo vecchi (%.0fmin > %.0fmin) → cedo al GPS (%s)",
