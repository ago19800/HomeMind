...
class HomeModel:
    def __init__(self, person_blacklist: list = None, motion_blacklist: list = None):
        ...
        self._alarm_extra_delay:  int  = 1    # secondi di attesa tra una partizione e l'altra
    ...
    def _load_config(self):
        ...
        try:
            ...
            if isinstance(_ap_raw, dict):
                ...
                # Leggi alarm_arm_mode come campo separato (salvato dalla UI)
                self._alarm_arm_mode = cfg.get("alarm_arm_mode", "armed_away")
            else:
                alarm_panel = ""
                self._alarm_arm_mode = cfg.get("alarm_arm_mode", "armed_away")
            ...
            _extra_delay = cfg.get("alarm_extra_delay", 1)
            try:
                self._alarm_extra_delay = max(0, int(_extra_delay))
            except (ValueError, TypeError):
                self._alarm_extra_delay = 1
            if self._alarm_extra_panels:
                logger.info("  alarm_extra_delay: %ds tra partizioni", self._alarm_extra_delay)
            ...
            for person_eid, pcfg in prox_cfg.items():
                ...
                if sensor_eid:
                    # ── Nuovi campi: zona progressiva + multi-fonte ──────────
                    _zone_y   = pcfg.get("zone_yellow_m", None)
                    _zone_y   = int(_zone_y) if _zone_y is not None else None
                    _zone_w   = int(pcfg.get("zone_yellow_wait_min", 5))
                    _wifi     = pcfg.get("wifi_tracker", "").strip()
                    _require  = int(pcfg.get("require", 1))
                    # Preserva last_near_ts dal build precedente se ancora valido.
                    # CRITICO: senza questo, ogni rebuild (ogni 2min) azzera lo sticky
                    # anche se il sensore non si è aggiornato ma la persona è ancora vicina.
                    import time as _pt
                    _prev_prox       = self.proximity_sensors.get(person_eid, {})
                    _prev_near_ts    = _prev_prox.get("last_near_ts", None)
                    _sticky_sec_init = 7200 if not stale_check else 300
                    _keep_near_ts    = (
                        _prev_near_ts is not None
                        and (_pt.time() - _prev_near_ts) < _sticky_sec_init
                    )
                    self.proximity_sensors[person_eid] = {
                        ...
                        "zone_yellow_m":       _zone_y,   # None = disattivata
                        "zone_yellow_wait":    _zone_w,   # minuti attesa zona gialla
                        "wifi_tracker":        _wifi,     # "" = non configurato
                        "require":             _require,  # fonti necessarie (1/2/3)
                        "current_m":           None,
                        "last_seen":           None,
                        "last_near_ts":        _prev_near_ts if _keep_near_ts else None,
                    }
                    if _zone_y:
                        logger.info("  proximity %s: zona_gialla=%dm attesa=%dmin",
                                    person_eid, _zone_y, _zone_w)
                    if _wifi:
                        logger.info("  proximity %s: wifi_tracker=%s require=%d",
                                    person_eid, _wifi, _require)
        ...
        for person_eid, prox in self.proximity_sensors.items():
            ...
            if sensor_eid in cache:
                ...
                try:
                    ...
                    import time as _t
                    if stale:
                        # stale_check=true e dato vecchio → disattiva
                        logger.warning("PROXIMITY init: %s = %.0fm dati VECCHI (%.0f min, max %d) — disattivato",
                                       person_name, dist_m, age_min if age_min else 0, max_age)
                        prox["current_m"] = None
                    elif not do_stale:
                        # stale_check=false: FIDATI DEL VALORE SEMPRE, qualunque età
                        # Invalida solo se unavailable/unknown (gestito dal try/except)
                        # Logica: se non ti sei mosso, il sensore resta a 0m — è corretto
                        prox["current_m"] = dist_m
                        prox["last_seen"] = _t.time() - (age_min * 60) if age_min else _t.time()
                        is_near = dist_m <= prox["threshold_m"]
                        prox["last_near_ts"] = _t.time() if is_near else None
                        age_str = f"{age_min:.0f} min fa" if age_min is not None else "età ignota"
                        logger.info("PROXIMITY init: %s = %.0fm (dati %s) → near=%s [stale_check=false, always trusted]",
                                    person_name, dist_m, age_str, is_near)
                    else:
                        # stale_check=true e dato fresco → logica normale
                        prox["current_m"] = dist_m
                        prox["last_seen"] = _t.time() - (age_min * 60) if age_min else _t.time()
                        is_near = dist_m <= prox["threshold_m"]
                        if is_near:
    ...
    def _zone(self, name: str, eid: str) -> str:
        kw = {
            ...
            "terrazzo":"esterno","balcone":"esterno","esterno":"esterno",
            "scala":"scala","corridoio":"corridoio","disimpegno":"corridoio",
            "ingresso":"ingresso","entrata":"ingresso","porta":"ingresso",
            "cucina":"cucina","salotto":"salotto","soggiorno":"salotto",
            "camera":"camera_da_letto","letto":"camera_da_letto","notte":"camera_da_letto",
            "bagno":"bagno","ufficio":"ufficio","studio":"ufficio","taverna":"taverna",
            "cantina":"cantina","lavanderia":"lavanderia","ripostiglio":"ripostiglio",
            "living":"salotto","kitchen":"cucina","bedroom":"camera_da_letto",
            "stair":"scala","hall":"corridoio","outdoor":"esterno",
        }
        text = (name + " " + eid).lower()
        for k, z in kw.items():
            if k in text:
    ...
    def _person_is_home(self, person_eid: str) -> bool:
        ...
        stale_check=false: fidati del valore SEMPRE (invalida solo se unavailable)
        stale_check=true:  invalida dopo max_age_min minuti (default)
        """
        import time as _t
        ...
        if prox:
            ...
            if current_m is not None:
                ...
                if not do_stale:
                    # stale_check=false: valore sempre affidabile
                    # Aggiorna last_near_ts per coerenza
                    if is_near:
                        prox["last_near_ts"] = _t.time()
                    else:
                        prox["last_near_ts"] = None
                    if is_near != gps:
                        data_age = (_t.time() - last_seen) / 60 if last_seen else 0
                        logger.info("Proximity override %s: dist=%.0fm (dati %.0fmin) → a_casa=%s (GPS=%s)",
                                    person_eid, current_m, data_age, is_near, gps)
                    return is_near

                # stale_check=true: controlla età
                data_age = (_t.time() - last_seen) if last_seen else 999999
                MAX_STALE_SEC = 5400 if not gps else 14400  # 90min se GPS=away, 4h se GPS=home
                if data_age < MAX_STALE_SEC:
                    if is_near:
                        prox["last_near_ts"] = _t.time()
                    else:
                        prox["last_near_ts"] = None
                    if is_near != gps:
                        logger.info("Proximity override %s (dati %.0fmin): dist=%.0fm → a_casa=%s (GPS=%s)",
                                    person_eid, data_age/60, current_m, is_near, gps)
                    return is_near
                else:
                    logger.info("Proximity %s: dati troppo vecchi (%.0fmin > %.0fmin) → GPS (%s)",
                                person_eid, data_age/60, MAX_STALE_SEC/60, gps)
    ...
    def everyone_away_gps(self) -> bool:
        """GPS-only check: True se tutti hanno GPS = not_home.
        Non usa proximity override — usato per armamento startup/rebuild.
        """
        if not self.persons:
            return False
        return all(
            not self._is_home_state(v.get("state", ""))
