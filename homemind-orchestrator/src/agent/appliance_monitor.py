...
class ApplianceMonitor:
    ...
    def status_message_single(self, name: str) -> str:
        """Ritorna lo stato di un singolo elettrodomestico per nome."""
        name_low = name.lower()
        found = None
        for app in self._appliances.values():
            if app.name.lower() == name_low or name_low in app.name.lower():
                found = app
                break
        if not found:
            return f"🔌 Elettrodomestico <b>{name}</b> non trovato nella configurazione."
        if found.is_running:
            dur = found.duration_str()
            return (f"🔌 <b>{found.name}</b>\n"
                    f"─────────────────────\n"
                    f"🔄 <b>In funzione</b> — da {dur}\n"
                    f"<i>🕐 {now_local().strftime(chr(37)+chr(72)+chr(58)+chr(37)+chr(77))}</i>")
        else:
            return (f"🔌 <b>{found.name}</b>\n"
                    f"─────────────────────\n"
                    f"✅ <b>Ferma</b>\n"
    ...
    def get_all_running(self) -> dict:
        """Ritorna dict {nome: stato} di tutti gli elettrodomestici in funzione."""
        result = {}
        for name, mon in self._monitors.items():
            if hasattr(mon, "_running") and mon._running:
                result[name] = "running"
            elif hasattr(mon, "is_running") and mon.is_running():
                result[name] = "running"
        return result
