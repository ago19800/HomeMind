...
class EnergyAnalyzer:
                    ...
                    try:
                        ...
                        if fval > 1000 and unit in ("kWh", "Wh", ""):
                            ...
                            if ieri_val is not None and ieri_val > 100:
                                ...
                                # Nessuno snapshot ieri — salva il raw per ora
                                # (il giorno dopo potrà calcolare il delta)
                                snap[cat] = fval
                                logger.info(
                                    "EnergyAnalyzer snapshot %s [cumulativo, no ieri] %s: %.3f",
                                    today_str, cat, fval
                                )
                        else:
                            snap[cat] = fval
    ...
    def _compute_averages(self, exclude: str = "") -> tuple[dict, dict]:
        ...
        averages = {
            cat: round(totals[cat] / counts[cat], 3)
