...
class AutomationEngine:
    def __init__(self, notifier=None, rest_client=None,
        ...
        self._entity_registry = entity_registry  # EntityRegistry per entità precise
    ...
    async def parse_automation(self, text: str, chat_id: str) -> Optional[dict]:
        ...
        # Usa EntityRegistry se disponibile (più preciso e completo)
        # Altrimenti fallback al loop inline sul cache
        if self._entity_registry:
            entities_str = self._entity_registry.get_context_for_ai(max_per_domain=30)
        else:
            entities_ctx = []
            for eid, s in cache.items():
                name  = s.get("attributes", {}).get("friendly_name", eid)
